"""Draft protocol-v1 canonicalization and golden-vector reference.
...
# idna.core otherwise imports the running interpreter's unicodedata module.
# Bind it once, before this module exposes any canonicalization operation. This
# permanent process-wide binding avoids the races caused by swapping a module
# global around individual calls.
idna_core.unicodedata = unicode_data
if idna_core.unicodedata is not unicode_data:
    raise RuntimeError("failed to bind IDNA validation to pinned Unicode data")
    ...
    while stack:
        ...
        if is_mapping:
            ...
            for key, child in reversed(entries):
                ...
                _nfc(key)
                stack.append((child, next_depth, False))
        else:
            entries = list(item)
            if len(entries) > MAX_ARRAY_ITEMS:
                raise CanonicalizationError(
...
def canonical_json(value: Any) -> bytes:
    ...
    try:
        _validate_value(value)
        result = _encode_canonical(value).encode("utf-8", errors="strict")
