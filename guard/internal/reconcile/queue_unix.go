...
func openQueue(config Config) (queueImpl, error) {
	...
	if err := q.withLock(context.Background(), func() error { return q.validateAll() }); err != nil {
		unix.Close(fd)
		return nil, err
	}
	return q, nil
...
func (q *unixQueue) claim(ctx context.Context, b Binding, now time.Time) (p.ReconciliationRecord, error) {
	...
	err := q.withLock(ctx, func() error {
		...
		sort.Slice(items, func(i, j int) bool {
			...
			return items[i].Record.ObservedAt < items[j].Record.ObservedAt
		})
		foundBlocked := false
...
func (q *unixQueue) fail(ctx context.Context, b Binding, id p.ReconciliationID, now time.Time, retryable bool) (p.ReconciliationRecord, error) {
	var result p.ReconciliationRecord
...
func (q *unixQueue) ack(ctx context.Context, b Binding, id p.ReconciliationID, receipt Receipt) (p.ReconciliationRecord, error) {
	...
	err := q.transition(ctx, b, id, func(env diskEnvelope) (p.ReconciliationRecord, error) {
		...
		if r.State == p.ReconciliationStateAcknowledged {
			if r.Acknowledgement != nil && r.Acknowledgement.ReceiptID == receipt.ReceiptID {
				result = r
				return r, nil
			}
			return r, ErrConflict
		...
		if err != nil || at.Before(timeMust(r.LastAttemptAt)) {
			return r, ErrRejected
		}
...
func (q *unixQueue) discard(ctx context.Context, b Binding, id p.ReconciliationID, now time.Time, authority p.DiscardAuthorityType, reason string, retention bool) (p.ReconciliationRecord, error) {
	var result p.ReconciliationRecord
	err := q.transition(ctx, b, id, func(env diskEnvelope) (p.ReconciliationRecord, error) {
		r := env.Record
		if r.State == p.ReconciliationStateSending || r.State == p.ReconciliationStateAcknowledged || r.State == p.ReconciliationStateDiscarded ||
			!safeReason.MatchString(reason) || (authority == p.DiscardAuthorityTypeOrganizationRetentionPolicy && !retention) ||
...
func (q *unixQueue) names() ([]string, error) {
	...
	var records []string
	for _, name := range names {
		if name == ".queue.lock" || isTempName(name) || isQuarantineName(name) {
			continue
		}
...
func (q *unixQueue) entries() ([]string, error) {
	...
	defer file.Close()
	names, err := file.Readdirnames(q.config.MaxRecords + 35)
	if err != nil && !errors.Is(err, io.EOF) {
		return nil, ErrUnsafePath
	}
	if len(names) == q.config.MaxRecords+35 {
		return nil, ErrQueueFull
	}
	return names, nil
}
func (q *unixQueue) usage() (int, int64, error) {
	...
	for _, n := range entries {
		...
		if !isRecordName(n) && !isQuarantineName(n) {
...
func (q *unixQueue) atomicWrite(ctx context.Context, name string, document []byte, expected *diskEnvelope) error {
	...
	if unix.Renameat(q.rootFD, tmp, q.rootFD, name) != nil {
		return ErrUnsafePath
	}
	keep = true
